...
#_fetchurl return the real data
#_ParseAndCheckTwitter change the real data to json
...
consumer_key    = 'kHDtH2n65CEwF63OvZZnw'
consumer_secret = '70FvZB6LDMqdTqgI2tlnM0BPTIe555J7L9AgtWfx4o'
access_token_key='334533325-Covhd3YFpPF2bo21OQpXngqHFkyTx6rvuAAsHB0A'
access_token_secret='Nu3I813QSlNi4lDr3y0FtnFsUzM69PJjtA7UwgRFRcI'
...
class Api(object):
  ...
  def Search_status(self,q,page=1,rpp=100,show_user="true"):
    parameters = {}
    parameters['q'] = q 
    parameters['rpp'] = rpp 
    parameters['page'] = page
    ...
    cur = con.cursor()
    cur.execute('CREATE TABLE IF NOT EXISTS twitter_status (u_id INTEGER PRIMARY KEY autoincrement,id integer ,status text,\
                                                user_name text,user_id integer,\
                                               created_at text,source text,profile_image_url)')
    con.commit()
    json = self._FetchUrl(url, parameters=parameters)
    data = ""
    try:
    ...
    if len(status)==0:
        ...
        for i in range(len(status)):
            sta=status[i]
            try:
                cur.execute("INSERT INTO twitter_status( id,status,user_name,user_id,created_at,source,profile_image_url) \
                            values ('%d','%s','%s','%d','%s','%s','%s')" % \
                                            (sta["id"],sta["text"],sta["from_user"],sta["from_user_id"],\
                                            sta["created_at"],sta["source"],sta["profile_image_url"]))
                con.commit()
            except Exception,e:
                print "twitter failed error:",e

    #cur.execute("Delete from status where u_id not in (select max(u_id) from status group by id)" )
    #con.commit()
    cur.close()
    con.close()
    return "true"
  ...
  def Search_users(self,q,page=1,per_page=20,include_entities="true"):
    parameters = {}
    parameters['q'] = q 
    parameters['rpp'] = per_page
    parameters['page'] = page
    ...
    con = sqlite3.connect('./twitter.db')
    cur = con.cursor()
    cur.execute('CREATE TABLE IF NOT EXISTS twitter_users (u_id INTEGER PRIMARY KEY autoincrement,id integer  ,name text,\
                                        screen_name text,time_zone text,\
                                        location text,description text,\
                                        statuses_count integer,friends_count integer,\
                                        profile_image_url text,created_at text,\
                                        status_id integer,status text,status_source text,status_time text,status_retweet_count integer)')
    con.commit()
    json = self._FetchUrl(url, parameters=parameters)
    information = self._ParseAndCheckTwitter(json)
    
    if len(information)==0:
        return "twitter_nodata"
    else:
        for i in range(len(information)):
            ...
            if info.get("status","none")!="none":
                ...
                try:
                    cur.execute("INSERT INTO twitter_users( id,name,screen_name,time_zone,location,description,\
                                            statuses_count,friends_count,profile_image_url,created_at,\
                                            status_id,status,status_source,status_time,status_retweet_count ) \
                            values ('%d','%s','%s','%s','%s','%s','%d','%d','%s','%s','%d','%s','%s','%s','%d')" % \
                                        (info["id"],info["name"],info["screen_name"],info["time_zone"],info['location'],info["description"],\
                                        info["statuses_count"],info["friends_count"],info["profile_image_url"],\
                                        info["created_at"],infostatus["id"],infostatus["text"],infostatus["source"],\
                                        infostatus["created_at"],infostatus["retweet_count"]))
                    con.commit()
                except Exception,e:#sqlite3.OperationalError:
                    #print e
                    pass
            else:
                try:
                    cur.execute("INSERT INTO twitter_users( id,name,screen_name,time_zone,location,description,\
                                ...
                                values ('%d','%s','%s','%s','%s','%s','%d','%d','%s','%s')" % \
                                            (info["id"],info["name"],info["ccreen_name"],info["time_zone"],info['location'],info["description"],\
                                            info["statuses_count"],info["friends_count"],info["profile_image_url"],\
                                            info["created_at"]))
                    con.commit()
                except Exception,e:#sqlite3.OperationalError:
...
class Twitter_Sql():
    ...
    def get_users(self, name, index=0, tbname = "twitter_users"):
        name = "%"+name+"%"
        sql = "select id, name, location, status from %s where name like '%s'" % (tbname, name)
        ...
        for i in range(len(ret)):
            ...
            for j in range(len(ret[i])):
                if j == 2:
                    new.append("")    #add gender
                if ret[i][j] == None:
                    new.append("")
                else:
                    new.append(ret[i][j])
            new.append("twitter.com")
            newret.append(new)
                
        return newret
                
    def get_status(self, name, index=0, tbname = "twitter_status"):
        try:
            ...
            try:
                ...
                print "get status encode error:",e
        name = "%"+name+"%"
        sql = "select user_id, user_name, id, status from %s where status like '%s'" % (tbname, name)
        #print sql
        self.cur.execute(sql)
        self.con.commit()
        if index >0:
    ...
    def clear_all(self, tbname = "twitter_users"):
        sql = "delete from %s" % tbname
        try:
            self.cur.execute(sql)
            self.con.commit() 
        except Exception,e:
            print "clear %s failed" % tbname,e
...
if __name__ == "__main__":
    ...
    #'''
    api_twitter=Api()
    api_twitter.Search_status(q=ke)
    #api_twitter.Search_users(q=ke,page=1)
    #test = Twitter_Sql()
    #print test.get_users("Pete Correale")
    #ke = raw_input(">")
    #ke = ke.decode('gbk')
    #print test.get_status(ke, 0)